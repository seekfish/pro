<?php 
	...
	// 选择数据库
	mysql_select_db("seek");
	// 创建插入语句
	$sql = "INSERT INTO mall_users (email, password) VALUES('$email', '$password')";
	// 执行SQL语句，返回执行结果，true表示执行成功，false表示执行失败
	$result = mysql_query($sql);

	// 判断是否注册成功
	if ($result) {
		$sql = "SELECT uid, email, score, level, createtime FROM mall_users WHERE email='$email'";
		$result = mysql_query($sql);
